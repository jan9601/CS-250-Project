...
if __name__ == "__main__":
    project_root = Path(__file__).resolve().parents[2]
    weather_path = project_root / "data" / "daily_weather_clean_table_grapes.csv"

    df = pd.read_csv(weather_path)
    df["date"] = pd.to_datetime(df["date"])

    season_start_date = "2024-04-01"
    as_of_date = pd.to_datetime("2024-07-15")
    ...
    result = predict_harvest(
        ...
        daily_weather=df[["date", "tmin", "tmax"]].to_dict(orient="records"),
