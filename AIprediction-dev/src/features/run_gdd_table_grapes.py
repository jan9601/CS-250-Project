...
def main():
    ...
    in_path = project_root / "data" / "daily_weather_clean_table_grapes.csv"
    out_path = project_root / "data" / "weather_with_gdd_table_grapes.csv"

    df = pd.read_csv(in_path)

    df = add_gdd_columns(df, base_temp_c=10.0)
    df = add_cumulative_gdd(df, season_start_month=4,season_start_day=1)
